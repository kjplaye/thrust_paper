...
\title{Causing the Effect: Localization, Mode Structure, and Mechanisms in the Unruh Phenomenon}
...
We motivate the construction of a driving source using an example pictured in Figure \ref{paz}.  The observer on the right at $(t,x) = (0,1)$ is named Wright and we also have an observer on the left at $(0,-1)$ named Lester.  A common causal ancestor exists in the past, Paz at $(-1,0)$. Paz emits an entangled pair of photons with frequency $\omega_k$ as part of a stream of photons that are the source, and cause, of the acceleration of Wright and of Lester.  The accelerations are thus correlated.

Now where we would have usually created a localized detector for Wright, such as an Unruh-DeWitt detector, and coupled it with the field, we instead seek to model the situation where Paz, a thruster, is coupled to the field. This inversion of effect and cause, of detector and thruster, is at the heart of our construction. Paz's emission makes the source of the acceleration manifest as particle injection, so that Wright will make sense of a guaranteed observation of a ``thrust particle'' with fixed peaked momentum $\omega_k$. Wright’s detection is entangled with Lester’s, so Lester will also observe a particle with momentum $\omega_k$, both by construction and due to wave function collapse.

Physically, Paz introduces a bilocal source term\footnote{This source is typically localized in time.  For our modes, it could be over a whole wedge, but via Stoke's thereom, we can integrate over a Cauchy surface and over a finite short time interval instead.} $J$,
\begin{equation}
...
\begin{equation}
 J(x,y) = f_L(x) f_R(y), 
\end{equation}
that injects the entangled pair of excitations, into the left wedge parameterized by $x$, and into the right wedge parameterized by $y$, using modes $f_L$ and $f_R$ respectively, with the product structure respecting the entanglement across the horizon. The bilocal form encodes the global entanglement, ensuring that any right-wedge excitation is correlated with a specific complementary mode in the left wedge.

This is a paired global-coherent version of the usual squeezing. To analyze a mode $f_R$ in the right wedge, we pair the mode with a complementary mode in the right wedge $f_L$.  This setup is fully compatible with the standard thermal vacuum description of the Unruh effect \footnote{$\theta$ is chosen to match $\alpha_k = \cosh{\theta}$ and $\beta_k = \sinh{\theta}$ so $\tanh{\theta} = e^{-2\pi\omega_k / a}$}.
\[
...
\begin{enumerate}
\item Lester emits a particle traveling back in time in Rindler coordinates\footnote{This is in Rindler time ordering, not a Minkowski causality violation}.
...
For the remainder of this note we will study the modes $f_R$ in the right wedge without mention of $f_L$, but we will operate in the context of a localized driving source in the right wedge that comes from a globally paired source.
