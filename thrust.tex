% TODO
%   * Figure out or rephrase the sections about divergence due to non-finite source activation
%   * Think harder about how the holographic section appears spectrally.
...
\]
Plugging this into equation (\ref{number}) we find a $(1,1)$-dimensional version of Plank's law\footnote{This is the average energy of the mode, not the average number of particles (see \cite{qspplanck}).  It is likely that we should replace $\mathbb{E}[\Delta N]$ with $\mathbb{E}[\Delta E]$}, and thus recover the Unruh radiation spectrum\footnote{Compare to equation (\ref{radeq}) and references \cite{unruh} and \cite{Frodden}.} from a thrust driven field.
\[
