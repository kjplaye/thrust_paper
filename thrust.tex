...
Much of this section including notation and convensions are drawn directly from the overview \cite{Frodden}.
...
\]
The same is true for $v$, $g(v)$, $p_v$, and $\hat{g}(v)$.  The full two dimensional transform of combinations of $f$ and $g$ is a linear combination of $\hat{f}(p_u) \delta(p_v)$ and $\hat{g}(p_v) \delta(p_u)$.  These are supported on the massless shell --- ``the momentum light cone''.
...
Let $\phi$ be a scalar field in the flat 2 dimensional Minkowski spacetime\footnote{Note that we only consider the scalar case since generalization to other fields is straightforward and does not add any intuition to the presentation.}.  We will consider $h^{(u)}_k$ and $h^{(u)}_k$ as driving $W$-event horizon sources
\begin{equation}
\label{ab}
...
We want to integrate $\rho$ on shell in momentum space, which for a massless source is the positive momentum part of the massless shell.  The two positive time horizons border $p_u <= 0$ and $p_v <= 0$, see Figure \ref{masslessshell}.  Proceeding to take the Fourier transform of the function $f(u)$\footnote{WLOG since $g(v)$ is of the same form.}, we drop $p_u$ to just $p$ for the time being to increase legibility.  We will continue to assume that $\omega_k$ and $a$ are positive. Let
\[
...
\begin{equation}  
  ...
  \begin{split}
    ...
    &=  \frac{\left|\Gamma\left(\frac{i\omega_k}{a} + 1\right)\right|^2}{2 \pi \sinh \left({\frac{\pi\omega_k}{a}}\right)} \int{1/|p|^3 dp}\\  
&=I(\omega_k) P
  \end{split}
\end{equation}
where the integrals are on the positive energy massless shell with contributions from $p_u$ on the left piece and $p_v$ on the right piece.  We factored out $P = \int{1/|p|^3}$ with a remaining $p$ independent positive real coefficient $I(\omega_k)$.

Without being more careful about the finite time and shape of the source we end up with inferred problems.  The integrals do not converge at zero where $P$ explodes.  But this infinity cancels when we compare the relative high frequency asymptotic spectral radiances $I(\omega_k)$.
