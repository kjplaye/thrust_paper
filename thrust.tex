...
To review how a uniformly accelerated observer perceives the Minkowski vacuum as a thermal bath, we construct the Unruh modes\cite{unruh1976notes}, analytic combinations of Rindler modes that are positive-frequency solutions with respect to Minkowski time.

From now on let $\omega_k = k > 0$. We define constants $\alpha_k$ and $\beta_k$ which satisfy $\alpha_k^2 - \beta_k^2 = 1$.  They will show up in future Bogoliubov relations, ensuring proper normalization, and describing particle creation in terms of $|\beta_k|^2$ which has a thermal character:
\begin{equation}
  \begin{aligned}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
These modes are positive-frequency with respect to Rindler time $\eta$ on the wedge $W$, but contain negative frequency components in Minkowski time $t$ on the left wedge\footnote{The time-like Killing vector $\partial_\eta = e^{a\xi} \partial_t$ when $t=0$ but flips negative on the left wedge \cite{frodden2018unruh,jacobson2005introduction}.}. See the middle column of Figure \ref{unruh_rainbow} for an illustration: the magnitude shown drops across the branch cut, whereas the alternative conjugate prescription would yield a jump upwards instead.  We can also see that the time derivative of the phase negates on the left wedge.
...
A natural physical interpretation is that a driving source exists which is the very agent or mechanism responsible for the observer’s acceleration. In this view, acceleration is not merely a kinematic or coordinate artifact but a manifestation of localized interactions or forces acting along the observer’s worldline. 

Figure \ref{emit_absorb} illustrates the situation with a particle composed of Rindler modes. The modes $r_k$ are left-moving, propagating toward the future horizon and interpreted as {\bf emission}; the $r_{-k}$ modes are right-moving, originating from the past horizon and interpreted as {\bf absorption}. These Rindler modes\footnote{The Unruh modes are a specific combination of the emission and absorption modes see equation (\ref{unruh_mode_def}).} are constructed as superpositions of Minkowski modes $\varphi_q$, effectively smeared across a range of frequencies.  This frequency mixing is evident in Figure \ref{unruh_rainbow}, where the modes blue-shift infinitely near the horizons and red-shift infinitely at spatial infinity, due to the geometry of the wedge.
...
Consider the two nested Rindler wedges $W_c \subseteq W_0$ shown in Figure \ref{restrict}. Let $r_q$ denote a Rindler mode\footnote{From now on we concentrate on fixed Rindler modes, but we can replace them with Unruh modes linearly to match Section 3.} associated with $W_0$, analytically continued to the entire Minkowski space. The gray-scale region indicates the full support of $r_q$, while the rainbow-colored segment shows its restriction to the sub-wedge $W_c$.
