...
\caption{Stokes phenomenon for the Parabolic Cylinder Function as $\sigma$ interpolates between $\infty$ and $0$.  The Stokes curve is passed at $\arg z = -\frac{\pi}{4}$.}
\label{stokes}
\end{figure}

\section{Conclusion}

This work reframes the Unruh phenomenon as a localized effect. By restricting Rindler modes to translated, reflected wedges, and their intersections, we demonstrated that the apparent thermal behavior could begin to appear as a localized driving source. We showed how the thermal properties could be partially eliminated as we localized the Rindler modes to get a spectrally peaked response. We then smoothly interpolated between the global thermal behavior and a compactly supported localized description using wave packets and parabolic cylinder functions.

While our analysis is grounded in flat spacetime, the equivalence principle offers a natural pathway to extending this framework to curved spacetimes. In particular, future work could be to apply this framework to Hawking radiation by modeling localized excitations near black hole horizons, as illustrated schematically in Figure \ref{rocket}, which depicts a localized thrust near a black hole horizon.
