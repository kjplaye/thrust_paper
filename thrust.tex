...
\begin{equation}
  \left|\left< \varphi_q N, r_k \right>\right|^2 \rightarrow \left\{ \begin{array}{lll}
    \frac{1}{4\pi a\omega_q}  \frac{z^{2\nu} \left| e^{-z^2} \right|}{\sinh \frac{\pi \omega_k}{a}}    & \text{as} & \sigma \to \infty     \vspace{10pt}
\\
    \frac{\omega_k}{2\pi a^2\omega_q} z^{-\nu - 1} & \text{as} & \sigma \to 0 \\
  \end{array}  \right.  
\end{equation}
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{stokes.png}
\caption{Stokes phenomenon for the Parabolic Cylinder Function as $\sigma$ interpolates between $\infty$ and $0$.  The Stokes curves is passed at $\arg z = -\frac{\pi}{4}$.}
\label{stokes}
\end{figure}

\section{Conclusion and Prediction}

This work reframes the Unruh effect as a localized, detector-based response to directed energy input, such as acceleration-induced coupling—rather than as a consequence of global horizon thermality. By restricting Rindler modes to sub-wedge regions and their intersections, motivated by modular theory, we demonstrate that apparent thermal behavior can emerge from non-local mode mixing rather than a thermal density matrix. Using parabolic cylinder functions, we construct smooth, spatially localized wave packets that interpolate between global and compact support, revealing how detector response depends sensitively on localization scale. Although our results are developed in flat spacetime, the equivalence principle suggests a natural extension to curved spacetimes. In particular, this framework may shed light on Hawking radiation by modeling localized excitations near black hole horizons, as illustrated in Figure \ref{rocket}.
