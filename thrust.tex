...
\end{aligned}
\]
where the first line assumes $Im(p) < 0$, the second line assumes $Im(p) > 0$ and the third line is a lie :P
CITE WOLFRAM ALPHA and maybe do the standard QFT contour integral trick(s).

We end up with a mode $\rho(p)$
FINALLY, TRY TO USE THE MODES AS DRIVING SOURCES OF THE INHOMOGENOUS KLIEN-GORDON EQUATION.  This seems to lead to inferred problems.  We end up with integrals that compute the complex magnitude like this computation of expected particle number.
\[
 \int_{-\infty}^\infty \frac {|\rho(p)|^2}{2p} = \int_{-\infty}^\infty p^{-3} dp
 \]
