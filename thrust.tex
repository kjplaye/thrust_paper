...
\abstract{We analyze the quantum field theoretic framework where Unruh radiation was first described, interpreting thrust as a driving source influencing the field. By conceptualizing thrust in this way, we provide a more intuitive mechanism for understanding radiation in accelerating frames.  Using the equivalence principle, we extend this framework to Hawking radiation as well, demonstrating how a common source-response relationship manifests in accelerating frames.}
...
\begin{equation}
\begin{split}
  L(p_u) & = -\int_0^{i\infty} \left(\frac{ias}{-p_u}\right)^\frac{i\omega_k}{a} \left(\frac{i}{-p_u}\right)ds \\
