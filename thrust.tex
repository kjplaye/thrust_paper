...
\title{Driving the Unruh Response: Entangled Excitations from Bilocal Sources}
\author[ ]{Kevin Player}
\affil[ ]{Software Engineering Institute}
\affil[ ]{Carnegie Mellon University}
\affil[ ]{\textit{kplayer@andrew.cmu.edu}}

\maketitle

\epigraph{The Unruh effect tells us that what we call particles is really just a matter of perspective.}{Lee Smolin}

\abstract{We present a simple model that connects the Unruh effect to non-thermal entangled excitations produced from a bilocal driving source motivated by squeezing interactions in quantum optics. These paired excitations can be thought of as an inertial microstates in the accelerated thermal Unruh ensemble. The model reverses the usual story; rather than entanglement being a consequence of acceleration, we demonstrate how an acceleration response can itself be engineered. We then construct localized driving modes that avoid the asymptotic Doppler tails responsible for thermality. Using modular automorphisms, we track mode and observer localizations across nested Rindler wedges and construct compact wave-packet approximations with parabolic cylinder functions. These provide a smooth interpolation from global thermal Rindler modes to fully localized nonthermal excitations, and outline a program for understanding vacuum correlations as causally driven, nonthermal responses of the field.}
...
Recent work has explored the subtlety of this thermality, examining detector locality, causal structure, and the entanglement-information tradeoffs that underpin Unruh correlations. Anastopoulos and Savvidou \cite{anastopoulos2012coherences} showed that while a single uniformly accelerated Unruh–DeWitt detector exhibits thermal fluctuations, correlations between two spatially separated detectors with the same acceleration are nonthermal. This indicates that the Unruh effect is fundamentally local, with information encoded in correlations between distant observers. Foo, Onoe, and Zych \cite{foo2020unruh} studied detectors in quantum superpositions of classical trajectories. Although each trajectory individually yields a thermal response, the superposed detector does not thermalize, demonstrating the importance of causal relations rather than acceleration alone.
...
We have presented a framework in which individual microstates of the thermal Unruh ensemble are realized as physically localized, source-driven excitations, making the role of acceleration explicit rather than purely kinematic. We consider localization via wedge-restricted Rindler/Unruh modes and then construct compact wave-packets via parabolic cylinder functions. This provides a smooth interpolation between global Rindler modes and localized spectral structures.

Our construction complements the standard detector-based interpretation by showing how apparent thermality can coexist with microstate-level descriptions.
