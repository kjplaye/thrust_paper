...
To come up with the functions $f_L$ and $f_R$, we consider a setup compatible with the standard thermal vacuum description\footnote{The form of this vacuum equation can be proven by recursively applying the Bogoliubov relations in equation (\ref{a_in_c}).} of the Unruh effect
\begin{equation}
...
The second issue is that the actual coherent state obtained from $J$ contains terms at higher particle numbers, see equation (\ref{one_fock}).  We either assume strongly that the state is exactly prepared with no higher terms, or that $\tanh{\theta_k}$ is sufficiently small, for high frequencies, as to make the higher terms negligible.
