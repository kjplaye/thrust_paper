...
Let the superscripts $(0)$, $(c)$, and $(M)$ represent the $W_0$, $W_c$, and Minkowski frames of reference respectively.  $(0c)$ represents a map from the Von Neumann algebras of $W_c$ to $W_0$ and $(cM)$ represents a map from the algebras of $M$ to $W_c$.

Since the analytically extended Rindler modes remain solutions of the Klein–Gordon equation, their inner products are preserved under continuation from the wedge to Minkowski space.  This allows us to directly compute from $M$ to $W_0$ Bogoliubov coefficients
\begin{equation}
  \left[ \begin{array}{l}
    a^{(0)}_k \\
    a^{(0)}_{-k} \\
