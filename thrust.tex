...
\end{equation}
where $\omega^2 = k^2$, covering the positive and negative frequency cases.
...
The following notations and results, (\ref{sinh}) through (\ref{radeq}), are taken from \cite{Frodden}. Let $W$ be the $z>|t|$ Rindler ``wedge'' with coordinates
\begin{equation}
...
The sources can originate from a coupling term, $\rho \phi$, added to the free scalar free Lagrangian.
\begin{equation}
\mathscr{L}_{driven} = \mathscr{L}_{original} + \rho\phi 
\end{equation}
where
\begin{equation}
  \mathscr{L}_{original} = -\frac{1}{2} \partial^\mu \phi \partial_\mu \phi - \frac{1}{2} \phi^2.
\end{equation}
