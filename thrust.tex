...
The construction of the absorption mode $f_R$ for Wright presents some physical challenges.  The first is that the actual coherent state obtained from $J$ contains terms at higher particle numbers.  We either assume strongly that the state is exactly prepared, or that $\tanh{\theta_k}$ is sufficiently small as to make the higher terms negligible.

The second challenge is that we are integrating over the entire Minkowski plane.  This challenge is met by noticing that our integral of eternal modes $f_R$ (and $f_L$) can be replaced by a finite time interval $|t + 1| < \delta$ supports around Paz's time $t=-1$ using Stokes' theorem. (see Figure \ref{paz_freq} (right picture)). This is also manifestly evident by the fact that the mode is translation invariant over time and space, $f_R(t,x) = f_R(-1,x+t+1)$, since it is a sum of right moving Minkowski modes. The coupling constant $\lambda$ multiplied by $\delta$ is chosen to match the vacuum equation (\ref{vacuum}) at $n_\omega = 1$.
...
An interesting side note is that Lester's proper time is flowing in reverse to Minkowski time.  So we can actually track the story\footnote{We can interchange these emission and absorption stories, Wright emits and Lester absorbs, but we would need to involve an observer in the future at $(t,x) = (1,0)$ for that.} as:
\begin{enumerate}
...
\end{enumerate}
The presence of virtual particle-antiparticle pairs in massive scalar fields further enriches this picture, where antiparticles are understood as particles moving ``back in time'', though this extension lies beyond the scope of the present note.
