...
To describe the physics from the point of view of a uniformly accelerating observer, we introduce Rindler coordinates \cite{Frodden,rindlerorig} covering a right wedge 
\begin{equation}
  W = \{(x,t) : x>|t|\}
\end{equation}
...
To address this, we introduce a driving source \cite{schwinger} \cite{ryder1996quantum}, a physical mechanism that excites the field and, indirectly, accounts for the observer's acceleration. This reframes the interpretation: the observed radiation is not spontaneous, but emerges as a coherent, dynamical response to an external influence. An illustration of this setup is shown in Figure \ref{rocket_inertial}. From this perspective, the apparent thermality arises not from intrinsic properties of the vacuum, but from an effective ignorance of the source's detailed structure and dynamics.
...
We will next formulate a method to interpolate between the thermal and and the localizes pictures using parabolic cylinder functions\cite{AbramowitzStegun1964}, see \cite{hashiba2021stokes} for a similar approach.

Let $N = e^{-\frac{1}{2} \frac{(x-t-\mu)^2}{\sigma^2}}$ be a Gaussian kernel that we will use to shape a wave packet\footnote{We multiply $\varphi$ by $N$ but we could just as easily multiply $r_k$ by $N$ for the same effect.}. Then using the parabolic cylindrical function $D_\nu(-z)$ we have
\begin{equation}
