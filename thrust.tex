...
\abstract{We connect the thermal Unruh effect to entangled, non-thermal excitations produced by a bilocal driving source modeled on squeezing interactions from quantum optics. These paired excitations can be thought of as inertial microstates in the accelerated thermal Unruh ensemble. Our construction reverses the usual narrative: instead of beginning with an accelerated observer and finding entangled excitations, we show how entangled emissions from a bilocal source can dynamically generate a portion of the accelerated response. We then form localized driving modes that avoid the asymptotic Doppler tails responsible for thermality. Using modular automorphisms, we track mode and observer localizations across nested Rindler wedges and construct compact wave-packet approximations with parabolic cylinder functions. These provide a smooth interpolation from wedge-supported thermal Rindler modes to fully localized non-thermal excitations, and outline a program for interpreting portions of the Unruh effect as source-driven, non-thermal responses.}
