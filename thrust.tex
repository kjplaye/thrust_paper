...
\end{aligned}
\end{equation}
where we choose $\log(-1) = -i \pi$ following \cite{Frodden}. Each $r_k$ with can be extended to the entire plane as
\begin{equation}
...
\caption{Phase of a Rindler mode $r_k$ with $k<0$ analyticlly extended to the whole $x/t$ or $u/v$ plane as ${(av)}^{\frac{i\omega_k}{a}}$.}
\label{analytic}
\end{figure}
