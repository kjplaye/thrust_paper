...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
These modes are positive-frequency with respect to Rindler time $\eta$ on the wedge $W$, but contain negative frequency components in Minkowski time $t$ on the left wedge. See the middle column of Figure \ref{unruh_rainbow} for an illustration: the magnitude shown drops across the branch cut, whereas the alternative conjugate prescription would yield a jump upwards instead.  We can also see from the figure that the phase evolves in the opposite temporal direction in the left wedge. The time-like Killing vector is positive on the right wedge $\partial_\eta = e^{a\xi} \partial_t$ when $t=0$, but flips to negative on the left wedge \cite{frodden2018unruh,jacobson2005introduction}. 
