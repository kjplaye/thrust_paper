...
Suppose we add the particles responsible for the thrust, $\mathcal{T}$, back into to our system.  The resulting larger system,  $\mathcal{B}$ = $\mathcal{A} + \mathcal{T}$, would no longer experience a net acceleration.  So the very meaning of net acceleration requires the separation of a system from its thrust.

The physical difference between $\mathcal{A}$ and $\mathcal{B}$ is the thrust $\mathcal{T}$.  In accordance with Landauer, the transitions follow this correspondence
\begin{enumerate}
\item $\mathcal{B} \rightarrow \mathcal{A}$ corresponds to {\bf ``Forgetting''}\\
\item $\mathcal{A} \rightarrow \mathcal{B}$ corresponds to {\bf ``Learning''}\\
\end{enumerate}
The 2nd transition is called a measurement.  The 1st transition is the opposite of measurement -- seemingly a measurement by an observer in $\mathcal{H}$
