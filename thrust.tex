...
Let $N = e^{-\frac{1}{2} \frac{(x-t-\mu)^2}{\sigma^2}}$ be a Gaussian kernel that we will use to shape a wave packet\footnote{We multiply $\varphi$ by $N$ but we could just as easily multiply $r_k$ by $N$ for the same effect.}. Then using the parabolic cylindrical function\cite{AbramowitzStegun1964} $D_\nu(-z)$ we have
\begin{equation}
