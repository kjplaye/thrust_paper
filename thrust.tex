...
\end{equation}
the expected number of excitations for a mode $r_k^{(c_2)}$ when expressed in the vacuum of $W{c_1}$, is invariant\footnote{Similar statements are true for reflected (diamond) wedges.} under changes in both $c_1$ and $c_2$.
...
\begin{equation}
  ...
  \label{chain}
\end{equation}
The concluding discovery is that each inclusion $W_{c_i} \subseteq W_{c_j}$ yields particle production with fixed squared Bogoliubov magnitude $|\beta_{kq}|^2$, implying that the expected number of particles remains constant across all nested wedge pairs, independent of the absolute values of $c_i$ or $c_j$.  We will see this independence in a future section where the green curve in Figure \ref{peaked}, for the complete beta function, is actually independant of the subwedge translation involved, and in the next ``Driving Sources'' section.
...
To further study the situation, consider the modulus squared inner product $\left|\left<r_q^{(0)}, r_k^{(c)} \right>\right|^2$, also known as the Bogoliubov $\left|\alpha^{(c \rightarrow 0)}_{kq}\right|^2$, from equation (\ref{bogoC0}).  We fix $q$ and view \footnote{$|\Gamma(ib)|^2 = \frac{\pi}{b \sinh \pi b}$}.
\begin{equation}
...
I thank Frodden and Valdés for their excellent exposition \cite{frodden2018unruh}, and Beisert for his insightful lecture notes \cite{beisert2012quantum}. I'm also grateful to Ben Commeau, Daniel Justice, and Edward Randtke for helpful discussions.
