...
Let $\widetilde{W}$ be the left Rindler wedge\footnote{Coordinates on this wedge are $t = -\frac{1}{a}e^{a\delta} \sinh(a\gamma)$, $x = -\frac{1}{a}e^{a\delta} \cosh(a\gamma)$; and $l_{\pm k} = \frac{1}{\sqrt{4 \pi \omega_k}} e^{i\omega_k(\gamma \pm \delta)}$.}, $x < -|t|$ with Rindler modes $l_{\pm k}$.  We define constants $\alpha_k$ and $\beta_k$ which satisfy $\alpha_k^2 - \beta_k^2 = 1$.  They will show up in future Bogoliubov relations and also in normalizations\footnote{The normalizations come from computing the Klein Gordon inner product on Minkowski space and comparing it to inner products on $W$ and $\widetilde{W}$.}. $\beta_k$ also describes particle creation in terms of $|\beta_k|^2$ which has a thermal character:
\begin{equation}
  \begin{aligned}
...
\begin{equation}
  \begin{array}{ll}
    \mu^R_{\pm k} = \alpha_k \frac{1}{\sqrt{4 \pi \omega_k}} (a(\mp t + x \pm  i \epsilon))^{\pm \frac{i \omega_k}{a}} & \hspace{20pt}
       {\mu^R_{\pm k}}_{|_W} \rightarrow \alpha_k r_{\pm k} \\
    \mu^L_{\pm k} = \alpha_k \frac{1}{\sqrt{4 \pi \omega_k}} (a(\mp t - x \pm  i \epsilon))^{\pm \frac{i \omega_k}{a}} & \hspace{20pt}
    {\mu^L_{\pm k}}_{|_{\widetilde{W}}} \rightarrow \alpha_k l_{\pm k} \\
  \end{array}
\end{equation}
We add an $i \epsilon$ prescription to dictate which branch of the log to take so that the modes are analytic and bounded on the $\mathfrak{I}(t) < 0$ half plane.  This makes the modes positive-frequency with respect to $t$.  Then another way of writing the Unruh modes is
\begin{equation}
\begin{aligned}
  \mu^R_k &= \alpha_k r_k + \beta_k l^*_{-k} \\
  \mu^L_k &= \alpha_k l_k + \beta_k r^*_{-k} \\
\end{aligned}
\label{unruh_mode_def}
\end{equation}
See the right column of Figure \ref{unruh_rainbow} for an illustration: the magnitude shown drops across the branch cut, whereas the alternative conjugate prescription would yield a jump upwards instead.
...
\caption{Spacetime diagrams of the $k>0$ mode functions $\left[\begin{array}{ccc} \varphi_k & \mu_k \\ \varphi_{-k} & \mu_{-k} \end{array} \right]$. Color encodes the phase; brightness indicates magnitude. The Unruh modes $\mu_k$ and $\mu_{-k}$ change magnitude across the log branch due to the interpretation of $\log(-1 \pm i\epsilon)$. The left-moving mode $\mu_k$ corresponds to emission; the right-moving $\mu_{-k}$ to absorption.}
\label{unruh_rainbow}
\end{figure}
