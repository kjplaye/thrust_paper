...
\begin{equation}
  \begin{aligned}
    ...
    \mathscr{L}_{sourced} & = \mathscr{L}_{free} + \frac{1}{2}\lambda \int f_L(x)f_R(y) \phi(x) \phi(y) \\
                        &= \mathscr{L}_{free} + \frac{1}{2}\lambda \left(q_{k}^{L\dagger} q_{-k}^{R\dagger} + q_{k}^{L} q_{-k}^{R} + q_{k}^{L\dagger} q_{-k}^{R} + q_{k}^{L} q_{-k}^{R\dagger} \right)
 \end{aligned}
 \label{bilocal}
\end{equation}
with coupling constant $\lambda$, normalized mode profiles $f_L$ and $f_R$ supported on the left and right wedges respectively, and creation\footnote{Match with equation (\ref{a_in_c}) where $q_{-k}^{R\dagger} = a_k^{(0)}$.} \footnote{The mixed terms vanish on the vacuum at leading order, so only $q_{k}^{L\dagger} q_{-k}^{R\dagger}$ contributes to the first $\ket{1}_L \ket{1}_R$ excitation.} operators $q_{k}^{L\dagger}$ and $q_{-k}^{R\dagger}$. This source construction is based on the standard classical source construction from QFT \cite{Schwinger_1966} \cite{ryder1996quantum} and is motivated by quadrature squeezed light generation in quantum optics \cite{gerry2023introductory}; it is a coupling of a classical function with the field as an interaction term smeared over $x$ and $y$. In this version $J$ squeezes the field creating correlations that manifest as entanglement at the level of a global quantum state. The product structure reflects this paired excitation.
...
\begin{equation}
  ...
  \label{one_fock}
\end{equation}
a controlled approximation of one term in the ensemble. Our source singles out the $\omega$ term, giving a selected ``slice'' of the full thermal ensemble. The source $J(x,y)$ does not contradict the thermal interpretation, but rather describes a specific ``microstate'' consistent with the broader thermal ensemble.
