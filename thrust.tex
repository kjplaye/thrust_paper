...
We want to integrate $\rho$ on shell in momentum space, which for a massless source is the positive time part of the light cone.  The two positive time horizons are given by $p_u < 0$ and $p_v < 0$, see Figure \ref{masslessshell}.  Proceeding to take the Fourier transform of the function $f(u)$\footnote{WLOG since $g(v)$ is of the same form.}, $p_u$ drops to just $p$ for the time being to increase legibility.  We will continue to assume that $w_k$ and $a$ are positive. Let
\[
