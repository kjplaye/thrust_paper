...
\end{equation}
where the right and left modes ($r_{\pm k}$ and $l_{\pm k}$) are understood to be zero outside of their respective wedges. See Figure \ref{unruh_rainbow} for an illustration of the Unruh modes. The magnitude shown jumps across the branch cut and conjugates the phase.
...
\begin{equation}
  ...
  \label{c_ladder}
\end{equation}
where we let $q$ take both positive {\it and negative} values $q=\pm k$. So that we can summarize the transform as
\begin{equation}
