...
Finally, we emphasize that our running example represents just one of many possible mechanisms for exchanging energy and correlations between wedges; for example, one could replace the entangled pair emission by a process in which the source itself remains entangled with a single emitted quantum, a scenario whose time-reverse counterpart naturally corresponds to a future-wedge observer, though these cases involve additional complications beyond the scope of our note.
...
Finally, within the framework presented here, the thermal signature observed in the Unruh effect can be interpreted as arising from a causal source in the constructed scenario. Experimentally, within the context of the present construction, this implies that the Unruh-like response could be associated with specific, identifiable, accelerating ``thrust particles'', rather than appearing isotropic and random. Conversely, the absence of a thermal Unruh signal in scenarios where a known accelerating source exists would provide experimental evidence supporting this framework.
