...
\begin{equation}
 ...
 \label{bilocal}
\end{equation}
This source construction is based on the standard classical source construction from QFT \cite{Schwinger_1966} \cite{ryder1996quantum}; It is a coupling of a classical function with the field as an interaction term $J\phi$. In this version $J$ acts coherently on the field shifting the modes in both wedges simultaneously, creating correlations that build entanglement at the level of a global quantum state. The product structure reflects this paired excitation, with $f_L$ and $f_R$ defining the mode profiles localized respectively in the left and right Rindler wedges. This is a paired global-coherent version of the usual vacuum squeezing.
...
The construction of the absorption function $f_R$ for Wright presents some physical challenges.  The first is that the actual coherent state obtained from $J$ contains terms at higher particle numbers, see equation (\ref{one_fock}).  We either assume strongly that the state is exactly prepared with no higher terms, or that $\tanh{\theta_k}$ is sufficiently small as to make the higher terms negligible.
...
Another detail is that $J$ is described in the Rindler wedge, not immediately on the past wedge, so Paz will need to couple to Unruh modes -- the analytically continued Rindler modes (see Section 2).  We actually form $f_R$ out of a linear combination of a left Unruh mode $\mu^{L*}_{k}$ and right Unruh mode $\mu^R_{-k}$ to get a function supported on just the right wedge, see equation (\ref{unruh_mode_def}). The Unruh modes undergo an infinite blue shift at the horizon, but are more well behaved at Wright and Paz's locations\footnote{Strangely, and unlike the component Unruh modes, $f_R$ is actually zero on the past wedge. As such, we just study the localization of functions on the right wedge in subsequent sections.}. In subsequent sections, we aim to localize $f_R$ away from the high-frequency, near-horizon behavior typical of thermal modes, making the physical realization of the source $J$ more plausible as a localized wave packet.
