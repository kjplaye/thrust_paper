...
We analytically continue\footnote{From the definitions and properties of $\sinh$ and $\cosh$, it follows that $a(\mp t + x) = e^{a(\pm \eta + \xi)}$.}  $r_k$ and $r_{-k}$ to the $(t,x)$ plane using the standard $i\epsilon$ prescription to define a specific branch of the log\footnote{Specifically $r_{\pm k} = e^{\pm \frac{i \omega_k}{a}\log a(\mp t + x \pm \epsilon)}$.}:
\begin{equation}
  \begin{aligned}
